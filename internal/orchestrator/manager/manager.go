	...
	go func() {
		for {
			...
			case <-m.refreshTicker.C:
				if err := m.RefreshNodes(); err != nil {
					log.Printf("Error refreshing nodes: %v", err)
				}
			case <-m.ctx.Done():
				return
			}
...
func (m *SwarmManager) InitSwarm(advertiseAddr string) (string, error) {
	...
	if err := m.RefreshNodes(); err != nil {
		log.Printf("Warning: failed to refresh nodes after swarm init: %v", err)
	}
...
func (m *SwarmManager) UpdateNodeLabels(nodeID string, labels map[string]string) error {
	...
	if err := m.RefreshNodes(); err != nil {
		log.Printf("Warning: failed to refresh nodes after label update: %v", err)
	}
...
func (m *SwarmManager) updateNodeAvailability(nodeID string, availability swarm.NodeAvailability) error {
	...
	// Refresh nodes after update
	if err := m.RefreshNodes(); err != nil {
		log.Printf("Warning: failed to refresh nodes after availability update: %v", err)
	}
...
func (m *SwarmManager) RemoveNode(nodeID string, force bool) error {
	...
	if err := m.RefreshNodes(); err != nil {
		log.Printf("Warning: failed to refresh nodes after node removal: %v", err)
	}
...
func (m *SwarmManager) UpdateService(serviceID string, def deployment.ServiceDefinition) error {
	...
	for _, warning := range response.Warnings {
		log.Printf("Warning during service update: %s", warning)
	}
...
func (m *SwarmManager) ListServices() ([]deployment.DeploymentStatus, error) {
	...
	for _, service := range services {
		...
		if err != nil {
			log.Printf("Warning: failed to get status for service %s: %v", service.ID, err)
			continue
		}
